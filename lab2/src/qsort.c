...
void parallel_qsort_sort (int *T, const int size)
{
         ...
         * sequential merging */
        int i,j,k,nthreads,chunk_size,nmerges,nlevels;
        nthreads = num_threads;
        chunk_size = size/nthreads;
        printf("chunk_size:%d\n", chunk_size);
        #pragma omp parallel for schedule(static, chunk_size) private(i) num_threads(nthreads)
        for (i = 0; i < nthreads; i++) {
                qsort(T+(i*chunk_size),chunk_size,sizeof(int),compare);
        }

        //nthreads (also # of chunks) is even
        nmerges = nthreads/2;
        nlevels = nthreads/2;
        for (k = 0; k < nlevels; k++) {
                for (j = 0; j < nmerges; j++) {
                        merge(T+(j*chunk_size*2),chunk_size);
                }
...
void parallel_qsort_sort1 (int *T, const int size)
{
        ...
        nlevels = nthreads/2;
        for (k = 0; k < nlevels; k++) {
                #pragma omp parallel for schedule(static, 2*chunk_size) private(j)
                for (j = 0; j < nmerges; j++) {
                        merge(T+(j*chunk_size*2),chunk_size);
                }
                chunk_size = chunk_size * 2;
...
int main (int argc, char **argv)
{
        ...
        if (argc != 3)
        {
                fprintf (stderr, "qsort N num_threads\n");
                exit (-1);
        }
